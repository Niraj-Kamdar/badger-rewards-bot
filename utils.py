...
def summary(rewards_tree):
    token_dist_data = {
        key: list(map(lambda x: x["totals"]["0x3472A5A71965499acd81997a54BBA8D852C6E53d"], val.values()))
        for key, val in rewards_tree["userData"].items() if "digg" not in key.lower()
    }
    
    return {
        key: _list_summary(val)
        for key, val in token_dist_data.items()
    }
